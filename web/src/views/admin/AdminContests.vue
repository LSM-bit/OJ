<!--
  AdminContests.vue - 后台比赛管理：全量比赛列表（含私有/未开始/已结束）
-->
<template>
  <div class="page">
    ...
    <el-table :data="pagedItems" v-loading="loading" height="calc(100dvh - 198px)">
      ...
      <el-table-column label="赛制" width="90">
        <template #default="{ row }">
          <el-tag size="small">{{ row.rule.toUpperCase() }}</el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" width="100">
        <template #default="{ row }">
          <el-button size="small" text @click="$router.push(`/contests/${row.id}`)">查看</el-button>
        </template>
      </el-table-column>
    </el-table>
...
async function load() {
  ...
  page.value = 1
  try {
    const r = await api.get('/admin/contests') as any
    items.value = r.items ?? []
  } finally {
    loading.value = false
  }
